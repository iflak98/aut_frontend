...
const functions = require('./search-service.js')
// const searchFileForFunctions = require('./search-service.js')
...
function getProviderMocks (ctorMockData) {
  ...
  paremeters.forEach(param => {
    ...
    const emport = this.imports.find(el => el.name === type);
    debugger// name , alias, moduleName
    // const funct= this.imports.find(el=>)
    ...
    // searchFileForFunctions(`${type}.ts`, '/src');
    if (mockFn)
    {
      const functionsContent = functions.map(func => `    ${func}(){}`).join('\n');
      providerMocks.mocks.push(Util.indent(`
        ...
        .replace(/\n\s*?\n/gm, '\n') // remove empty lines
        .replace(/\{\s+?}\s*?/gm, `{  hee ${functionsContent}  }\n`) // ...{\n} to ...{}
      );
    }
